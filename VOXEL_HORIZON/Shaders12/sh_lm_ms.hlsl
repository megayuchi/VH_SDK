...
StructuredBuffer<D3DPLLMVERTEX> g_Vertices : register(t3);
...
//MeshletID     |   0  1  2  |   0  1  2  |    0  1    |    0  1    |   0  1  2  3 |  0  1  2  3  |
    ...
    while (vertexID < meshlet.IndexedVertexNum)
    {
		...
		// Vertex Transform
        D3DPLLMVERTEX vertex = g_Vertices[IndexedVertex];

    	VS_INPUT_LM input = (VS_INPUT_LM)0;
        input.Pos = float4(vertex.Pos, 1);
        input.Normal = float3(vertex.Normal);
        input.Tangent = float3(vertex.Tangent);
        input.TexCoordDiffuse = vertex.TexCoordDiffuse;
        input.TexCoordLightMap = vertex.TexCoordLightMap;
		...
		pOutVertexList[vertexID].TexCoordLightMap = input.TexCoordLightMap;
        pOutVertexList[vertexID].Diffuse = float4(1, 1, 1, 1);
#ifdef SHOW_MESHLET
        pOutVertexList[vertexID].Diffuse = float4(g_PalettedColor[meshltID % 16], 1);
#endif
		pOutVertexList[vertexID].NormalColor.rgb = (NormalWorld * 0.5f) + 0.5f;
        pOutVertexList[vertexID].NormalColor.a = 1;
