...
{
//	const float2 adj_uv = float2(0.5 / WIDTH, 0.5 / HEIGHT);
	...
	float4	ColorSum = texSrc.Sample(samplerClampLinear, texCoord);
	for (uint i = 0; i < 8; i++)
	{
		ColorSum += texSrc.Sample(samplerClampLinear, texCoord + texCoordOffset9Sample[i].xy);
	}
	float4	OutColor = ColorSum / 9.0;
	
	//float4	OutColor = texSrc.Sample(samplerClampLinear, texCoord);
