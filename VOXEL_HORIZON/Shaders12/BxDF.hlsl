...
namespace MaterialType
{
    ...
    static int Matte = 1;      // Lambertian scattering
    static int Mirror = 2;     // Specular reflector that isn't modified by the Fresnel equations.
    static int Effect = 3;     // No Lighting, No Shadow, No Reflection
    static int AnalyticalCheckerboardTexture = 4;
}
...
namespace BxDF {
    ...
    float3 Fresnel(in float3 F0, in float cos_thetai)
    {
		...
		return saturate(F0 + (1 - F0) * pow(abs(1 - cos_thetai), 5));
    }
    ...
    namespace Specular {
        ...
        namespace Transmission {
            ...
            {
                // refract
                float ior = 0.925;    // ±¼ÀýÀ² 0 - 1
                wt = refract(-V, N, ior);   // refract
