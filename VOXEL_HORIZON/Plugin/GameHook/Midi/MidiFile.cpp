...
namespace smf {
	...
	unsigned int MidiFile::GetMidiData(MIDI_MESSAGE* pOutMessageList, unsigned int  dwMaxBufferCount)
	{
		...
		int tpq = getTPQ();
		int bpm = getTicksPerQuarterNote();
		...
		for (i = 0; i < getNumTracks(); i++)
		{
			...
			for (j = 0; j < (int)m_events[i]->size(); j++) 
			{
				...
				writeVLValue((*m_events[i])[j].tick, trackdata); 
				/*
				(0) Note Off Event 0x8c noteNumber, velocity
				...
				(1) Note On Event 0x9c noteNumber, velocity
				...
				(5) Channel Aftertouch Event 0xDc amount
				...
				(6) Pitch Bend Event 0XEc value LSB, value MSB
				*/
				MIDI_MESSAGE	note = {};
				...
				if (msg.isNoteOn())
				{
					...
					int key = (*m_events[i])[j].getKeyNumber();
					int vel = (*m_events[i])[j].getVelocity();
				...
				if (bMustWrite)
				{
					if (MidiNoteCount >= dwMaxBufferCount)
						__debugbreak();
