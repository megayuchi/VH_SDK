...
#ifdef STEREO_RENDER
RWTexture2DArray<float> TexBuffer : register(u0);
#else
...
Texture2D Atmospheretex : register(t3);
...
cbuffer CONSTANT_BUFFER_GEN_SKY : register(b0)
{
	uint2	Res;
	float2	ResRcp;
...
void GenerateSky(uint3 groupID : SV_GroupID, uint3 dispatchThreadId : SV_DispatchThreadID)
{
	...
	float4 ray_view = float4(
		(((2.0f * (float)CurPixel.x) / (float)Res.x) - 1.0f) * DecompProj[ArrayIndex].rcp_m11,
		-(((2.0f * (float)CurPixel.y) / (float)Res.y) - 1.0f) * DecompProj[ArrayIndex].rcp_m22,
		1.0, 0.0);
	
	float4 ray_world = mul(ray_view, matViewInvArray[ArrayIndex]);
	...
	float4 bg = Atmospheretex.SampleLevel(samplerClampLinear, (float2)CurPixel / (float2)Res, 0, int2(0, 0));
	...
	if(fogAmount > 0.965)
	{
		...
		TexBuffer[DestCoord] = fragColor_v;
		return;
	}
...
/*
[numthreads(1024, 1, 1)]
void GenerateSkyOnFrameBuffer(uint3 groupID : SV_GroupID, uint3 dispatchThreadId : SV_DispatchThreadID)
{
	float4 fragColor_v, bloom_v, alphaness_v, cloudDistance_v;
...
#ifdef STEREO_RENDER
	float depth = DepthTex.Load(uint4(CurPixel, ArrayIndex, 0), int2(0, 0));
#else
	float depth = DepthTex.Load(uint3(CurPixel, 0), int2(0, 0));
#endif
	if (depth < 1.0)
		return;
