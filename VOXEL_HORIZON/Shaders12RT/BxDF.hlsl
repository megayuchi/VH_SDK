...
SamplerState	samplerWrap_Point	: register(s2);
...
namespace MaterialType
{
	enum Type
	{
        ...
        Effect,     // No Lighting, No Shadow, No Reflection
    ...
    namespace DirectLighting
    {
            ...
            in float3 L)
        {
            ...
            if (!inShadow && NoL > 0)
            {
                ...
                // toon shading
                float2	toonTexCoord = float2(NoL, 0.5);
				float4	toonColor = g_texToonTable.SampleLevel(samplerClamp, toonTexCoord, 0);
                //float3 toonColor = float3(0, 1, 0);
                directLighting = toonColor.rgb *  Radiance * (directDiffuse + directSpecular);
            }
