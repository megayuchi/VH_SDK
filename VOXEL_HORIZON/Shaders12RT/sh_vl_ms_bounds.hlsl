...
//MeshletID     |   0  1  2  |   0  1  2  |    0  1    |    0  1    |   0  1  2  3 |  0  1  2  3  |
    ...
    if (threadID < 12)
    {
        ...
        // 탄젠트 벡터 계산
        float T = cross(N, float3(1, 0, 0));    // 1,0,0 과 외적
        float T_len = length(T);
        if (T_len > -0.005 && T_len < 0.005f)   // T가 1,0,0이면 외적 결과는 0벡터이므로
        {
            T = cross(N, float3(0, 1, 0));  // 0,1,0과 외적해서 다른 탄젠트 벡터를 구한다.
        }
        ...
        for (uint i = 0; i < 3; i++)
        {
            ...
            pOutVertexList[BoundsVertexID].Normal = N;
            pOutVertexList[BoundsVertexID].Tangent = T;
...
PS_TARGET psDefault(PS_INPUT input)
{
    ...
    output.Color1 = float4(input.Normal * 0.5 + 0.5, 0);
